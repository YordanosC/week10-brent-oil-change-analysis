...
def calculate_event_impact(event, date, data):
    ...
    try:
        price_before_1m = data.loc[event_date - timedelta(days=30), 'Price']
        price_after_1m = data.loc[event_date + timedelta(days=30), 'Price']
        change_1m = ((price_after_1m - price_before_1m) / price_before_1m) * 100
    except KeyError:
        change_1m = None

    try:
        price_before_3m = data.loc[event_date - timedelta(days=90), 'Price']
        price_after_3m = data.loc[event_date + timedelta(days=90), 'Price']
        change_3m = ((price_after_3m - price_before_3m) / price_before_3m) * 100
    except KeyError:
        change_3m = None

    try:
        price_before_6m = data.loc[event_date - timedelta(days=180), 'Price']
        price_after_6m = data.loc[event_date + timedelta(days=180), 'Price']
